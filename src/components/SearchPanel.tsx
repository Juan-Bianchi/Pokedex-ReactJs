...
const SearchPanel = ({ onFilter, handleAccordion }: SearchPanelProps) => {
  ...
  const onMaxWeightChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    minWeight && e.target.value && minWeight > Number(e.target.value)
      ? setIsEnabled(false)
      : setIsEnabled(true);
    e.target.value === ""
      ? handleMaxWeigth(null)
      : handleMaxWeigth(Number(e.target.value));
  };

  const onMinWeightChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    maxWeight && e.target.value && Number(e.target.value) > maxWeight
      ? setIsEnabled(false)
      : setIsEnabled(true);
    e.target.value === ""
      ? handleMinWeigth(null)
      : handleMinWeigth(Number(e.target.value));
  };
  ...
  const resetFilters = (
    ...
    const checkboxes = panelRef.current?.querySelectorAll<HTMLInputElement>(
      'input[type="checkbox"]',
    );
    checkboxes?.forEach((checkbox) => (checkbox.checked = false));
    const radios = panelRef.current?.querySelectorAll<HTMLInputElement>(
      'input[type="radio"]',
    );
  ...
  return (
    <form
      ref={panelRef}
      className="w-full flex flex-col justify-center content-around pragmatica g-3 text-[#363b81]"
    >
      <div className="w-full mb-4 gap-3 flex flex-col">
        ...
        <div className="flex flex-wrap justify-normal content-center gap-2 w-full border-b-[3px] border-slate-400">
          {checks?.pokemon_v2_pokemoncolor &&
            checks.pokemon_v2_pokemoncolor.map((color) => (
              <div className="flex justify-stretch " key={color.id}>
                <input
                  className="bg-gray-50 checked:after:bg-[#363b81] border-[#363b81] focus:ring-3 focus:ring-[#363b81] h-4 w-4 rounded relative top-1 hover:cursor-pointer"
                  type="radio"
                  name={"colors"}
                ...
                />
                <label
                  className="inline-block pl-[0.15rem]  hover:cursor-pointer pb-5"
                  htmlFor={color.id.toString().concat("color")}
                >
                  {color.name
                    .charAt(0)
                    .toUpperCase()
                    .concat(color.name.slice(1))}
                </label>
              </div>
            ))}
        </div>
            ...
            checks.pokemon_v2_type.map((type) => (
              <div key={type.id}>
                <input
                  ...
                  onChange={(e) =>
                    e.target.checked
                      ? handleAddType(type.name)
                      : handleRemoveType(type.name)
                  }
                />
                <label
                  htmlFor={type.id.toString().concat("type")}
                  className="inline-block pl-[0.15rem]  hover:cursor-pointer pb-5"
                >
      ...
      <div className="flex flex-wrap justify-end gap-4">
        <button
          className={`btn-blue drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]`}
        ...
        <button
          disabled={!isEnabled}
          className={`${isEnabled ? "btn-blue drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]" : "btn-disabled"}`}
          onClick={(e) =>
            handleSubmit(e, color, types, minWeight, maxWeight, isABaby)
          }
        >
          Apply Filters
        </button>
