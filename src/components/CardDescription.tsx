...
const CardDescription = ({ pokemonId }: CardDescriptionProps) => {
  ...
  const finalImageLink: string = pokemon
          ...
          <div className="overflow-hidden flex flex-col justify-center items-center w-[90%] h-fit gap-7 bg-red-600 bg-opacity-50 rounded-2xl outline outline-slate-400 -outline-offset-8 p-1 py-4">
            <div className="flex flex-wrap gap-3">
              <div className="z-10 flex flex-wrap justify-center items-center gap-2 mb-3">
                <p
                  className={`text-[#363b81] text-center pragmatica-bold block drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)] px-2 ${pokemon.name.length <= 17 ? "text-4xl" : pokemon.name.length <= 25 ? "text-3xl" : "text-2xl"}`}
                >
                  {pokemon.name
                    .charAt(0)
                    .toUpperCase()
            ...
            <div className="w-[90%] flex flex-col md:flex-row justify-around items-center gap-3">
              <div className={`w-[55%] flex justify-between`}>
                ...
                />
                <div
                  className={`flex flex-col flex-wrap justify-center items-start ms-3 relative gap-1 w-[150px] scale-100 start-0`}
                >
                  {pokemon.pokemon_v2_pokemontypes.map((pokeType) => (
                    <div
                      className="iconBig rounded-full flex flex-col justify-center items-center"
              ...
              <div className="flex flex-col justify-center items-center text-[#363b81] gap-3 text-2xl">
                ...
                <div className="flex justify-between w-full">
                  ...
                  <span>
                    {
                      pokemon.pokemon_v2_pokemonspecy.pokemon_v2_pokemoncolor
                        .name
                    }
                  </span>
                </div>
                <div className="flex justify-between w-full">
                  ...
                  <span>
                    {pokemon.pokemon_v2_pokemonspecy.is_baby ? " Yes" : " No"}
                  </span>
                </div>
                {pokemon.pokemon_v2_pokemonspecy.pokemon_v2_evolutionchain.pokemon_v2_pokemonspecies.filter(
                  (poke) => poke.evolves_from_species_id === pokemon.id,
                ).length === 0 ? (
                    ...
                    <div className="flex flex-col justify-around items-end gap-3">
                      {pokemon.pokemon_v2_pokemonspecy.pokemon_v2_evolutionchain.pokemon_v2_pokemonspecies
                        ...
                        .map((poke) => (
                          ...
                          >
                            <ButtonRouter
                              endpoint={`/description/${poke.id}`}
                              spanText={poke.name
                                .charAt(0)
                                .toUpperCase()
                                .concat(poke.name.slice(1))}
                            />
                          </div>
                        ))}
                    </div>
